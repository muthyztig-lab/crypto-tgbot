"""
Telegram-бот аналізу криптовалют та оцінки ризиків. Версія 3.

Можливості:
- Вибір валюти кнопками або введенням тикера/назви
- Таймфрейми графіка від 1 години до 1 року
- Лінійний та свічковий графік, рівні підтримки/опору
- Ризик-бал 0..100, RSI, волатильність, тренд, Fear & Greed Index
- Сигнал LONG / SHORT з рівнями Stop Loss / Take Profit (ATR)
- Огляд усього ринку (/market) та топ руху за 24г (/top)
- Обране зі сповіщеннями при зміні ціни на заданий % (/favorites)
- Автовидалення старих повідомлень при виборі іншої монети
- Дані без API-ключів (CoinGecko), час оновлення з точністю до секунди
- Без емодзі: вся графіка — згенеровані SVG-картки

Запуск:
    1. Створіть бота через @BotFather у Telegram, отримайте токен
    2. Вставте токен у файл config.py (BOT_TOKEN = "...")
...
from analysis import full_analysis, support_resistance, updated_at
...
from signals import long_short_signal
...
# Sentry — моніторинг помилок (опційно, лише якщо заданий SENTRY_DSN)
...
# Пер-юзер обмеження частоти (free/pro ліміти)
dp.message.middleware(RateLimitMiddleware())
dp.callback_query.middleware(RateLimitMiddleware())

# Пороги сповіщень, які можна обрати кнопками (%)
ALERT_THRESHOLDS = [0.5, 1, 2, 3, 5, 10]

# ---------------------------------------------------------------------------
# Стан чату (у пам'яті):
#   coin     — поточна монета
#   tf       — поточний таймфрейм
#   mode     — режим графіка: "line" / "candle"
#   messages — id ВСІХ повідомлень розмови (бота і користувача),
#              які видаляються при зміні монети / поверненні в меню,
#              щоб чат не засмічувався
# ---------------------------------------------------------------------------
chat_state: dict = {}
MAX_CHAT_STATES = 5000  # обмеження, щоб словник станів не ріс без меж


def _state(chat_id: int) -> dict:
    st = chat_state.get(chat_id)
    if st is None:
        # Захист від необмеженого зростання памʼяті: якщо станів забагато,
        # прибираємо найстаріший (FIFO — звичайний dict зберігає порядок вставки)
...
async def _cleanup_messages(chat_id: int) -> None:
    ...
    for mid in st["messages"]:
        try:
            ...
            pass  # повідомлення вже видалене або застаре (>48 год)
    st["messages"] = []
...
def analysis_keyboard(coin_id: str, tf_key: str, mode: str) -> InlineKeyboardMarkup:
    ...
    # Два ряди таймфреймів: активний позначено дужками
    tf_rows = []
    ...
    # Перемикач режиму графіка — свічки доступні для ВСІХ таймфреймів
...
async def send_analysis(chat_id: int, coin_id: str, tf_key: str = None,
    ...
    # Дані (мережеві виклики — в окремому потоці, щоб не блокувати бота)
    ...
    if ohlc:
        # у свічковому режимі рівні підтримки/опору рахуємо з самих свічок,
        # щоб лінії збігалися з графіком
        a["support"], a["resistance"] = support_resistance([c[4] for c in ohlc])
    png = svg_to_png_bytes(build_analysis_svg(a, mode, ohlc, fng))

    # Дані готові — тепер чистимо старі повідомлення, щоб чат не засмічувався
    # (видаляємо ПІСЛЯ рендера: якщо сталася помилка, чат не лишиться порожнім)
...
async def cmd_start(message: Message):
    _track(message.chat.id, message)  # команду користувача теж приберемо
    # реєструємо користувача (+ реферал із deep-link: /start <referrer_id>)
    ref = 0
...
async def _send_error(chat_id: int, e: Exception) -> None:
    ...
    if isinstance(e, DataUnavailable):
        # DataUnavailable — це наш власний, безпечний для показу текст
        text = str(e)
    else:
        # Не показуємо користувачу внутрішні деталі винятку — лише в лог
...
# ===========================================================================
# PRO-функції: on-chain ризик, деривативи, тех.сигнали, бектест, новини,
# портфель, AI, алерти-правила, watchlist, монетизація, мова
# ===========================================================================
...
# ===========================================================================
# Текстовий пошук монети
# ===========================================================================

@dp.message(F.text)
async def on_text(message: Message):
    _track(message.chat.id, message)  # текст користувача теж приберемо
...
# ===========================================================================
# Запуск: бот + фонова система сповіщень
# ===========================================================================

async def main():
    await db.init()  # створюємо БД та таблиці, якщо їх немає
