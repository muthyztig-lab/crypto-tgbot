"""
БЛОК 5 (частина) — новини та сентимент.

Працює БЕЗ ключів через RSS (CoinTelegraph). Якщо заданий
CRYPTOPANIC_API_KEY — додатково тягне стрічку CryptoPanic із голосами
(bullish/bearish) для грубого сентименту.

Повертає список заголовків + наївну оцінку сентименту за ключовими словами.
"""
...
def _from_rss(limit: int) -> list:
    ...
    for url in RSS_FEEDS:
        try:
            ...
            # Беремо лише блоки <item>/<entry> (новини), а не <channel>/<image>
            # заголовки самої стрічки — інакше у список потрапляє назва сайту.
