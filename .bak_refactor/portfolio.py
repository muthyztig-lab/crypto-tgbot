"""
БЛОК 6 — портфель (read-only, ручне введення позицій).

Користувач додає позиції командою /addcoin <тикер> <к-сть> [ціна_купівлі].
Бот рахує поточну вартість, P&L по кожній позиції та сумарно, а також
алокацію (частку кожної монети). Ціни — через CoinGecko (без ключів).

Підключення біржі/гаманця по API лишається опційним розширенням; тут —
надійний ручний облік, який працює одразу.
"""

import db
from market_data import get_simple_prices
...
async def compute(user_id: int) -> dict:
    ...
    prices = get_simple_prices(coin_ids)  # sync; виклик у to_thread із бота
    ...
    # алокація (частка від поточної вартості)
    for p in positions:
