"""
Кеш із Redis (якщо заданий REDIS_URL) та м'яким fallback у пам'ять.

Навмисно простий синхронний інтерфейс get/set із TTL — для кешування
відповідей зовнішніх API (новини, on-chain, деривативи), щоб не впиратися
в ліміти та пришвидшити відповіді. Якщо Redis недоступний — усе одно
працює через локальний словник (як і раніше).
"""
...
def set(key: str, value, ttl: int = 60) -> None:
    ...
    if len(_mem) > _MEM_MAX:
        # прибираємо найстаріший за часом закінчення
