"""
Картки для нових функцій (on-chain ризик, деривативи, конфлюенс-сигнали,
бектест, портфель, новини).

Усе будується з примітивів, які підтримує і cairosvg, і власний
растеризатор на Pillow (pil_raster): rect, text, polygon. Жодних <line>.

Базовий конструктор build_kv_card(title, subtitle, sections, accent) робить
картку з заголовком і кількома секціями (підзаголовок + рядки ключ/значення
або список тексту), тому всі нові картки виходять однаковими й охайними.
"""
...
def build_onchain_card(r: dict, time_str: str) -> str:
    ...
    # смуга ризику як окремий блок через rows? Простіше додати у facts
