"""
Налаштування з оточення (env) — БЕЗ секретів у коді.

Усі ключі опціональні: якщо не задані, відповідні фічі м'яко вимикаються
(бот працює, але показує підказку «додайте ключ»). Токен бота лишається
у config.py і тут НЕ дублюється.

Можна задати через змінні оточення або файл .env поряд із ботом
(див. .env.example).
"""
...
# --- Моніторинг / інфраструктура ---------------------------------------
SENTRY_DSN = _get("SENTRY_DSN")
REDIS_URL = _get("REDIS_URL")  # напр. redis://localhost:6379/0

# --- Монетизація --------------------------------------------------------
# Ціна Pro у Telegram Stars (XTR) — оплата без зовнішнього провайдера.
PRO_PRICE_STARS = int(_get("PRO_PRICE_STARS", "299") or "299")
PRO_DAYS = int(_get("PRO_DAYS", "30") or "30")
# Необов'язковий провайдер для оплати картками (Stripe тощо)
PAYMENT_PROVIDER_TOKEN = _get("PAYMENT_PROVIDER_TOKEN")

# --- Ліміти (rate limiting) --------------------------------------------
# Безкоштовний vs Pro: дозволених важких запитів за вікно
...
# Скільки алертів дозволено (free / pro)
...
def feature_status() -> dict:
    ...
    return {
        ...
        "news": True,  # RSS працює без ключа; CryptoPanic покращує
